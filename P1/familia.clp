...
; Se ha usado la plantilla proporcionada en PRADO y
; se han comentado las reglas pregunta y pregunta2 para la realización
; del ejercicio

; Preguntamos por la persona M
(defrule pregunta_M
   (declare (salience 10000))
   =>
   (printout t "Dime el nombre de una persona (p.e Rosalia): " crlf)
   (assert (M (read)))
)
...
(defrule pregunta_R
   ...
   =>
   (printout t "Dime el tipo de relación (HIJO PADRE ABUELO NIETO HERMANO ESPOSO PRIMO TIO SOBRINO CUNIADO YERNO SUEGRO): " crlf)
   (assert (R (read)))
)

; Buscamos las relaciones masculinas...
(defrule buscar_relacion1
   (M ?p)
   (R ?t)
   (Relacion (tipo ?t) (sujeto ?p) (objeto ?x))
   (hombre ?x)
   (test (neq ?p ?x))
   =>
   (assert (encontrado ?p ?t ?x))
)

; Y las femeninas
(defrule buscar_relacion2
   (M ?p)
   (R ?t)
   (Relacion (tipo ?t) (sujeto ?p) (objeto ?x))
   (mujer ?x)
   ...
   (test (neq ?p ?x))
   =>
   (assert (encontrado ?p ?t2 ?x))
)
; y las añadimos a la "lista de coincidencias"

; De momento no hemos encontrado ninguna relación, asi que declaramos nuestra "lista de coincidencias"
; como vacía
(defrule declarar_vacio
   =>
   (assert (VACIO))
)

(defrule descartar_vacio
   (encontrado ? ? ?)
   ?x <- (VACIO)
   =>
   (retract ?x)
...
; Listamos cada una de las relaciones encontradas por pantalla
(defrule listar_relaciones
   ?r <- (encontrado ?p ?t ?x)
   =>
   (printout t ?x " es " ?t " de " ?p crlf)
   (retract ?r)
)

; Si la "lista de coincidencias", lo imprime por pantalla
(defrule no_ha_encontrado
   (M ?m)
   (R ?r)
   (VACIO)
   =>
   (printout t "Nadie es " ?r "/A de " ?m crlf)
)